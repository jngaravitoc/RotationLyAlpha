...
anisotropy in the integrated line flux, in the escape fraction or the
average number of scatterings. 
We also provide an analytical expression for the outgoing \lya spectrum of
a rotating cloud which accounts for the velocity and viewing angle
trends observed in our Monte Carlo simulations.
\end{abstract}
...
There are three key results that allows us to build an analytical description to
...
to each other, making the scattering process proceed identical as in
the static case.  
...
\begin{equation}
v_{\rm b}(b,\phi,i)=V_{\rm max}\frac{\sqrt{R^2 -s^2}}{R}\cos i  \hs \cos a,
\end{equation} where the factor $\cos i$ accounts for the projection onto the $x-y$ plane, and the factor $\cos a$ for the subsequent projection onto the line-of-sight. The {\it right panel} of Figure~\ref{fig:scheme} shows that this angle $a$ can be computed from
%
\begin{equation}
\tan [90^{\circ}- a]=\frac{c}{d}=\frac{ b\sin \phi}{\sqrt{R^2 -b^2}}.
\end{equation}
