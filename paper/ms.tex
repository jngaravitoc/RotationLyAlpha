...
\begin{figure}
  ...
  \caption{The two small circles show the two different spherical
    ...
    hacer las dos circulos pequenos punteados.}
    \label{fig:OCspheres}} 
\end{figure}
...
level of anisotropy induced by rotation does not surpasses the $5\%$
level. We performed the same test with all the models and we do not
fina any significant anisotropic effect.


We explore other kind of anisotropy by study the effect
of radiation sources distributed with a different symmetry than the
gas. To this end we select a subset of the photons in the homogeneous
models such as they are emitted from off-centered spheres with respect
to the gas distribution.

Figure \ref{fig:OCspheres} shows the two spherical regions we
define to test for this effect. Each has a radius of $0.5R$, where $R$
is the radius of the gaseus sphere. Provided that there is an observer
located along the $\hat{j}$ direction, we pick two possible positions
to center the spheres along the $\hat{k}$ axis: $\pm C\hat{k}$. In
...
\caption{\ly profiles for the off-center positions for all of our models, Dashed/solid
