...
\author{ Juan N. Garavito-Camargo and Jaime E. Forero-Romero}
...
the intensity at the line center increase with rotational velocity. 
For homogeneously distributed sources the line becomes single peaked 
at rotational velocities larger than the line width in the static case.
...
CLARA \citep{CLARA} and \citep{DijkstraKramer}.   
...
\end{center}
    \caption{2D histogram of $N_{\rm scatt}$ vs $x$. The upper (lower) panels
      ...
      corresponds to the static case and right panels corresponds to
...
values of $\tau_{\rm H} = (10^{5}, 10^{6}, 10^{7})$ respectively and
$V_{\mathrm{max}}=300$ \kms.


From Figure \ref{fig:Nscatt} for the homogeneous
distribution there is a decrease of $\langle N_{\rm
...
$V_{\rm{max}}=300\kms$ we find that $\langle N_{\rm
  ...
  case.  In contrast the analytic solution for the homogeneous
...
\caption{Triple peaked profiles for three different off-center positions
  ...
  line) and for different rotational velocities. And $\theta$ represents the usual
  polar angle.
...
and \lya photons leaking though low column density holes, as in \citep{Behrens2014}.
 The same argument applies to galaxies in the group IV in which the flux
...
\item The line width increases with rotational velocity. The line with
  ...
  the radiative transfer results it is $\lambda_{\rm c}=3.9\pm0.041$ and
  $\lambda_{\rm h}=1.9\pm0.062$ for the central and homogeneous source
...
The reduction in the number of scatterings decreases the probability
...
The data and source code and instructions to
replicate the results of this paper can be found
here {\texttt{https://github.com/jngaravitoc/RotationLyAlpha}}.
Thanks to the IPython community \citep{IPython} and to the matplotlib
community \citep{matplotlib}.
