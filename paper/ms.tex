...
sphere. Rotation increases by a factor of $2$ the escape fraction for
...
static spheres with centrally distributed sources $\langle N_{\rm
...
a single peak profile, if in this single peak there is an asymmetry it 
can be explained with the outflow/inflow model \cite{Verhamme2008, Dijkstra06}.
 But if it is not the case now they can be understanding with rotation, 
 in particular in this sample there are single peaks with a strong asymmetry 
 and there are peaks that this asymmetry its not notorious.
 
Between the 91 LAEs 8 reveal profiles with a central peak and two smaller
peaks at the redder and bluer side, this also could be explained by our
off-center model when higher velocities are involved in rotation.
...
gas bulk rotation is the principal effect governing the galaxy. For this reason 
