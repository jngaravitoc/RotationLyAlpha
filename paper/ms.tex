...
\end{center}
    \caption{2D histogram of $N_{\rm scatt}$ vs $x$. The upper (lower) panels
      ...
      $V_{\rm max}=300km/s$. The color scale is logarithmic on the
...
\ly photon far from the line's center. The result is that he peak maxima
shifts from the center as the amount of neutral hydrogen
increases. This can be precisely quantifyied in for the static slab
with central sources, in that case the maxima position are related to
the optical depth and the temperature as $x_{m}=\pm 1.066(a\tau_{\rm
  H})^{1/3}$ \citep{Harrington73}.
...
\begin{figure}
  \includegraphics[scale=0.35]{f8.eps}
...
\begin{figure*}
\includegraphics[scale=0.35]{f12.eps}
