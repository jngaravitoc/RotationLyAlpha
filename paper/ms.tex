...
at the line's centre increases with rotational velocity. In the case of homogeneously distributed
...
radiation sources located off-centre, the line morphology presents a
...
\subsection{Off-Centred emission}

\begin{figure}
  \includegraphics[scale=0.35]{OF_spheres.png}
  \caption{The two solid circles show the two different spherical
    off-centred emitting  regions. The angular velocity vector is
    defined to be along the $\hat{\i}$ direction and the observer is
    along the $\hat{\j}$ direction (where the reader is) (THE ANGLE
    THETA IS WRONG. IT IS THE ANGLE WITH THE OBSERVER, RIGHT?). 
    \label{fig:OCspheres}} 
\end{figure}
...
symmetry than the gas. We can select photons emitted in off-centred spheres 
with respect to the gas distribution and check whether this produces
asymetries in the line.
...
direction, such that the emitting sphere centred on $+C\hat{\i}$ moves
away from the observer and the sphere centred on $+C\hat{\i}$ moves
...
In the the off-center emission the variations  in the $\mu$ distribution
...
have outgoing directions aligned with the observer. The most
interesting effect is that emission is that in  some models ($\tau =
...
The results for the off-centered spheres placed closed to the center
at $C=\pm 0.25R$ do not display the features discussed above for the
spheres at $C=\pm0.5R$. 
...
\caption{\ly profiles for the off-center positions. Dashed/solide
...
in our off-centre emission models with rotation. 
...
additional condition of an off-centred emission. 
...
off-centred emission can result into triple peaked lines. 
