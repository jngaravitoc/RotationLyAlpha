...
towards the observer. We consider $C=0.5 R$. 
...
In the off-center emission the variations in the $\mu$ distribution
...
have outgoing directions aligned with the observer ($\vert \mu \vert > 
...
central model the line gets broader as the rotational velocity also 
...
in detail in the Sec.\ref{sec:3p}
...
As mentioned in Sec\ref{sec:off-center} Off-centered emission shows signs of three peak 
profiles, to analyse this in more detail we use the same method of the spheres
explained in that section. We select spheres at $C=\pm 0.25R, 0.5R, 0.75R$ with a radii of
$0.25R$ for all the models, for those who present signs of three peak profiles such as 
$(V_{max}=50kms^{1} \& \tau_{H}=10^{5}), (V_{max}=100kms^{1} \& \tau_{H}=10^{6}), 
(V_{max}=200kms^{1} \& \tau_{H}=10^{7})$ we made another run with $\tau_{\gamma}=10^{6}$ 
...
Another requirement is that a high optical must be balanced with a high velocity 
