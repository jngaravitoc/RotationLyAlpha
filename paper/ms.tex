...
$\alpha$ line in galaxies. We model a galaxy as a sphere with a
...
explained since we are in a regime where the condition for the
...
have to possible centers positions $C$ along the $\hat{\j}$ direction:
...
towards the observer. We consider two values for $C=(0.25,0.50)R$. 
...
a the observer. This is expressed by 
...
In the off-centre emission the variations in the $\mu$ distribution
$\mu$ increase with the rotational velocity. For $V_{\rm max}= 50$\kms
 $\Delta \mu \leq 7\%$; $V_{\rm max}=100$\kms, $\Delta \mu\leq 12\%$
and $V_{\rm max}=200$\kms, $\Delta\mu\leq 15\%$ (NOTE: regardless of
$\tau$??). This implies a strong dependence of the observed flux on
the relative position of an observer.  In contrast,the central and
homogeneous source distributions only show a weak anisotropy at the level of
$\Delta\mu\leq 3\%$.  

Figure \ref{fig:OC_profiles} show the spectra for the two off-centred
spheres located at $C=\pm0.5R$.  In this case we only use photons that
have outgoing directions aligned with the observer ($\vert \mu \vert > 0.9$ $\&$ $\vert \mu \vert < 1.0]$). The most
interesting effect is that emission in  some models ($\tau =
10^6, V=100,200$\kms and $\tau = 10^7, V=200,300$\kms) the line
presents three peaks (blue-shifted,centred, red-shifted). However, we
do not find any asymmetry in the line produced by the off-center
emission. As in the homogeneous / central model the line gets broader
as the rotational velocity increases. 

The results for the off-centred spheres placed closer to the centre
at $C=\pm 0.25R$ do not display such a stronger features discussed above for the
spheres at $C=\pm0.5R$, there is only one model which slightly shows a 
three peak morphology with $\mathrm{V=300km s^{1}}$ and $\tau = 10^{7}$. 


\begin{figure}
\includegraphics[scale=0.35]{mu_OC.png} \caption{Flux angular 
  dependency for off-center distribution. Different line widths
  illustrate different rotational velocities.\label{fig:OCflux}} 
\end{figure}
...
As mentioned in Sec 3.4 Off-centered emission shows signs of three peak 
profiles, to analyse this in more detail we use the same method of the spheres
explained in Sec 3.4. We select spheres at $C=\pm 0.25, 0.5, 0.75$ for all the 
models, for those who present signs of three peak profiles such as $(V_{max}=50kms^{1} 
\& \tau_{H}=10^{5}), (V_{max}=100kms^{1} \& \tau_{H}=10^{6}), (V_{max}=200kms^{1}
\& \tau_{H}=10^{7})$ we made another run with $\tau_{\gamma}=10^{6}$ with the aim 
of get a stronger profile. 
...
peak arise for all of our models with $\tau_{\gamma}=10^{6}$. The main effect of
the rotational velocity is to give rise to the central peak. If the rotational velocity is 
high also the optical depth have to be high in order to compensate the height of the wings 
of the line with the central peak. 

Finally we take into account the finite spectral resolution to see if three peaks 
profiles produced by rotation could be observed. Fig.\ref{fig:3p_res} show that even
with a low resolution a three-peak profile would be observed.	
...
the morphology of the \ly line in star forming
...
off-centered emission can result into triple peaked lines. 
...
in the morphology of the \ly emission line, we estimate the
range of this effects for simplified models.


\section*{Acknowledgements}

JNGC thanks the support of Universidad de los Andes given as TA
Also ISAAC 2012 summer school which provide computer time where some of this 
models where ran.
