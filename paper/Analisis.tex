...
in this section will be expressed in terms of frequencies expressed in
velocities. 


Next, we  quantify the line by its full width at half maximum
(FWHM) and the peak positions. In the next subsection we take a look
at the escape fraction. We conclude the section by estimating the
...
In Figure ~\ref{figure:maximumsvsvelocities} we present the position
...
If we now study the effect of the optical depth $\tau$ in the maxima
position $X_{m}$ Figure~\ref{figure:maximavsOD}, we found that as the
optical depth increase the maximum position increase and this is a
well-known result (include references), we found that this result is
