...
while the line center rises. This procudes the impresion that, as the
...
In order to gain a deeper understanding of these results we 2D
...
a function of rotational velocity. This Figure illustrates how our
reasoning in the previous paragraph is correct. The escape fraction
barely changes for the central source distribution, while it rises
as the velocity $V_{\rm max}$ increases for the homogeneous case. The
velocity has a higher relative impact in the models with low optical
depth $\tau_{\rm H}=10^5$,$10^{6}$, where it can raise from $xxx$ in
the static case up to $xxx$ at $V_{\rm rot}=300$\kms.

In Figure \ref{fig:efvsNeufeld} we put these results in the context of
the analytical solution for the infinite slab \citep{Neufeld90} shown
by the continous line. In this setup the analytic solution depends
solely on the product $(a\tau_{\rm   H})^{1/3}\tau_{a}$. This valid
only in the limits $a\tau_{\rm   H}\gg 1$. The dashed lines in Figure
\ref{fig:efvsNeufeld} corespond to the cases of different
velocities. We observe that in all cases the escape fraction is higher
than in the slab configuration. We also note that for $\tau_{\rm H}=10^{5}$
the escape fraction does not increase as expected, This is
due to the fact that the condition $a\tau_{h}\gg 1$ does not hold. 
