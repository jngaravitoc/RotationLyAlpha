...
that observers at different directions do not present an angular 
dependency, in particular they would infer luminosities
different from the isotropical value only at levels $< 3\%$. 

In the case of the Off-center model the 
%But with the set up of the off-center models
%where the galaxy is rotating around the  $\hat{\textbf{z}}$ axis and the observer is 
%located at $\mathrm{\theta = \pi/2}$ we do find and angular dependency for both  
%off-center distributions, this is shown in Fig. \ref{fig:OCflux}. The bold line  
...
the resulting profiles show more interesting morphologies than in the
$\pm 0.25R$ case.
...
% BUSCAR asimetrias enhttps://pod51044.outlook.com/owa/#path=/mail
% observaciones, Kulas, YAMADA  off-center emission 
% See steidel et al (2011) findings.
