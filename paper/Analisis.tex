...
$\alpha$ line in galaxies. We model a galaxy as as sphere with an
homogeneous mixture of dust and hydrogen at a constant
temperature. These spheres have a solid-body rotation with
maximum velocities in the range $0-300$ \kms. The neutral
hydrogen optical depths are in the range $\tau_{\rm
...
Lyman alpha photons ecapine in the line center increase with
rotational velocity. higher rotational velocities increase the width
of each peak in the outgoing line profile while it also increases the
amount of Lyman alpha photons escaping in the line. For large
...
sphere. Rotation increses by a factor of $2$ the escape fraction for
...
double and triple peak lines. This work demonstrates that gas bulk
...
$\mathrm{\theta = \pi/2}$. The first result we get is that 
the angular dependency is almost the same for all the models
with different $\tau_{H}$ and $V_{max}$. Both spheres located 
at $R = 0.25$ and $R = 0.5$ show variation in the flux just at
values $< 14 \%$ this results are shown in \ref{fig:OCflux}.
...
the resulting profiles show new interesting morphologies than in the
$\pm 0.25R$ case.
...
is presented. Those off-center spheres located at the positive side of 
$\hat{\textbf{x}}$ (solid lines) in Fig.\ref{fig:OCspheres} have a slightly
shift to redder part of the spectra while those located at the negative 
side of $\hat{\textbf{x}}$ are shifted to the bluer part, the model in 
which this is most notorious is for $R = 0.25$, $V_{max} = 50$ and $\tau=10^{5}$.


\begin{figure*}
\includegraphics[scale=0.3]{OC_profiles.png}
\caption{Off-center distribution $\mathrm{Ly}\alpha$ profiles, 
off-center positions.
...
the line presents three peaks (blue-shifted, centered, red-shifted). In order 
to gain a better interpretation of this three peaks trend we run this models
 with $N_{\gamma} = 10^6$.
...
Gas bulk rotation has a noticeable effect on the morphology of the
 \ly line. In this section we discuss the observational implications 
 of these findings. 

We start by a quantitative discussion of the line morphology. Here we
follow a classification of $Ly\alpha$ profiles made by
...
They classified the observed lines into four groups. In Group I the
red peak is stronger than the blue, For our porpoises Group II and Group II
with the blue peak stronger are the same, Group IV contains double-peak
profiles with roughly the same flux and finally group IV present multi-peaked 
lines.

Neither Group I nor II  and III can be fully explained by rotation. However,
 some comments about some of the galaxies in this groups must be done, in 
 particular for those galaxies in which the flux in between the two peaks is
 high rotation must be present due to the fact this is until now the most 
 efficient mechanism which produces this effect.
 
The same argument explained above applies to group IV in which the flux
in between of the two peaks is high ( $ \geq 70 \%$ of the highest peak), 
concerning group V in which multiple-peaks appear homogeneous and central
models can not reproduce such a profiles. While the off-center model could 
generate similar profiles which is the case of (put reference when the fig.
is ready) when tree peaks could appear but with roughly the same flux, this
multiple-peaks still need a better comprehension of the gas kinematics.

An other interesting sample of LAEs was presented by \cite{Yamada2012} 
for 91 LAEs at $z=3.1$. From that sample 31 LAEs ($\sim 33 \%$) present 
a single peak profile, if this single peak is present an asymmetry it 
can be explained with the outflow model \cite{Verhamme2008} due to the fact that 
we don't find any asymmetry due to rotation in  both homogeneous and central
models, while in the off-center model ---. But if it is not the 
case (put LAEs name for those that do not present asymmetry) now they can
be understanding with rotation.

Even in profiles where the double-peaked is presented rotation also could 
be present as it is seen in Fig.\ref{fig:differentvelocities} where rotation
makes that the central part of the line is higher than in profiles where 
there is no rotation. This could be the case of 13 LAEs in the sample of 
\cite{Kulas12} where there is a double-peak profile with similar heigh and 
a central notorious part.

Until now when single peaked profiles are present we associate this to
the presence of inflows and outflows due to the asymmetry of the line
, but in the case in which the profile have symmetry we could say that
rotation in the principal effect governing the galaxy. For this reason 
a detailed study involving inflows/outflows and rotation kinematics would 
give as better understanding of the $\mathrm{Ly} \alpha$ profiles we 
measured.
